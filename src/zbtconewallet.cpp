...
CzBTCONEWallet::CzBTCONEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbtcone seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzBTCONESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezBTCONESeed_deprecated()) {
                    LogPrintf("%s: Updated zBTCONE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbtcone seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbtcone wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBTCONEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezBTCONECount(nCountLastUsed);
    else if (!walletdb.ReadzBTCONECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBTCONEWallet::Lock()
{
    seedMaster = 0;
}

void CzBTCONEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBTCONEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozBTCONE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBTCONEWallet::LoadMintPoolFromDB()
{
...
void CzBTCONEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBTCONEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBTCONEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbtconeTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBTCONEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozBTCONE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbtconeTracker which also adds to database
    pwalletMain->zbtconeTracker->Add(dMint, true);
...
void CzBTCONEWallet::SeedTozBTCONE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBTCONEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBTCONEWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WritezBTCONECount(nCountLastUsed);
...
void CzBTCONEWallet::GenerateDeterministiczBTCONE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBTCONEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBTCONEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
