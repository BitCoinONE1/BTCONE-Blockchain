...
#include "zbtconechain.h"

#include "denomination_functions.h"
#include "libzerocoin/Denominations.h"
#include "zbtconewallet.h"
...
/**
 * Fees smaller than this (in ubtcone) are considered zero fee (for transaction creation)
...
{
    return zbtconeTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckzBTCONE = GetBoolArg("-zapwallettxes", false);
    if (fCheckzBTCONE)
        zbtconeTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zbtcone
            if (fCheckzBTCONE && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zbtconeTracker->GetMints(true);
    ...
    return zbtconeTracker->GetBalance(false, false);
}
...
{
    return zbtconeTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zbtconeTracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-btconestake", true)) {
    ...
    //zBTCONE
    if (GetBoolArg("-zbtconestake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zBTCONE set once per update interval
        bool fUpdate = false;
        ...
        set<CMintMeta> setMints = zbtconeTracker->ListMints(true, true, fUpdate);
        for (auto meta : setMints) {
            if (meta.hashStake == 0) {
                ...
                if (GetMint(meta.hashSerial, mint)) {
                    ...
                    zbtconeTracker->UpdateState(meta);
                }
            }
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZbtconeBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 BTCONE collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 BTCONE collateral
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IszBTCONE());
    ...
    if (!txNew.vin[0].scriptSig.IsZerocoinSpend()) {
        ...
        for (const CTxOut& out : txNew.vout) {
            ...
            if (!zbtconeTracker->HasPubcoinHash(hashPubcoin))
                return error("%s: could not find pubcoinhash %s in tracker", __func__, hashPubcoin.GetHex());

            CMintMeta meta = zbtconeTracker->GetMetaFromPubcoin(hashPubcoin);
            meta.txid = txNew.GetHash();
            meta.nHeight = chainActive.Height() + 1;
            if (!zbtconeTracker->UpdateState(meta))
...
void CWallet::AutoZeromint()
{
    ...
    // zBTCONE are integers > 0, so we can't mint 10% of 9 BTCONE
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zBTCONE\n", nBalance);
        return;
    }

    // Percentage of zBTCONE we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zBTCONE (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zBTCONE amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zBTCONE amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zBTCONE We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough BTCONE to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zBTCONE. Current percentage of zBTCONE: %lf%%\n",
...
bool CWallet::CreatezBTCONEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministiczBTCONE(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreatezBTCONEOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zbtcone output", __func__);
    ...
    // Sign if these are bitcoinone outputs - NOTE that zBTCONE outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), zBTCONE_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), zBTCONE_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), zBTCONE_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zBTCONE privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), zBTCONE_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a BTCONE transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), zBTCONE_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus(_("The transaction did not verify"), zBTCONE_BAD_SERIALIZATION);
            return false;
        }

        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            //Tried to spend an already spent zBTCONE
            receipt.SetStatus(_("The coin spend has been used"), zBTCONE_SPENT_USED_zBTCONE);

            uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
            if (!zbtconeTracker->HasSerialHash(hashSerial))
                return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

            CMintMeta meta = zbtconeTracker->Get(hashSerial);
            meta.isUsed = true;
            if (!zbtconeTracker->UpdateState(meta))
        ...
        receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), zBTCONE_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus(_("Spend Valid"), zBTCONE_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = zBTCONE_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zBTCONE"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = zBTCONE_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBTCONE transaction
    vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        setMints = zbtconeTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zBTCONE mints to use in this spend
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = zBTCONE_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zBTCONE require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to bitcoinone address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), zBTCONE_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), zBTCONE_SPEND_OKAY); // Everything okay
...
string CWallet::ResetMintZerocoin()
{
    ...
    set<CMintMeta> setMints = zbtconeTracker->ListMints(false, false, true);
    ...
    for (CMintMeta mint : vMintsMissing) {
        deletions++;
        if (!zbtconeTracker->Archive(mint))
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zbtconeTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zbtconeTracker->UpdateZerocoinMint(mint);
        }
...
void CWallet::ZPivBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozbtconebackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozbtconebackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozbtconebackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zbtconebackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zbtconebackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = zBTCONE_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", zBTCONE_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = zBTCONE_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zbtconeTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", zBTCONE_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", zBTCONE_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zbtconeTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zbtconeTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", zBTCONE_SPEND_OKAY);  // When we reach this point spending zBTCONE was successful
...
{
    CMintMeta meta;
    if (!zbtconeTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zbtconeTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zbtconeTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zbtconeTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zbtconeTracker->UpdateState(meta);
    } else {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zbtconeTracker->Get(hashSerial);
    zbtconeTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
