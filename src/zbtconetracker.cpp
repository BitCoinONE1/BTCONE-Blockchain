...
CzBTCONETracker::CzBTCONETracker(std::string strWalletFile)
{
...
CzBTCONETracker::~CzBTCONETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBTCONETracker::Init()
{
...
bool CzBTCONETracker::Archive(CMintMeta& meta)
{
...
bool CzBTCONETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBTCONETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBTCONETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBTCONETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBTCONETracker::GetSerialHashes()
{
...
CAmount CzBTCONETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBTCONETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBTCONETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBTCONETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBTCONETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBTCONETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBTCONETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBTCONETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBTCONETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBTCONETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBTCONETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBTCONETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBTCONETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBTCONETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBTCONETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBTCONETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBTCONETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbtcone from DB\n", __func__, listDeterministicDB.size());
    }
