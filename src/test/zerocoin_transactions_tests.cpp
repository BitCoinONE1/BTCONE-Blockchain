...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zbtconeTracker = unique_ptr<CzBTCONETracker>(new CzBTCONETracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == zBTCONE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == zBTCONE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
