...
// The zBTCONE block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBTCONE is a hash of the serial
...
{
    //Create an output returning the zBTCONE that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreatezBTCONEOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBTCONE output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBTCONE", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreatezBTCONEOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBTCONE output", __func__);
...
{
    CzBTCONETracker* zbtconeTracker = pwallet->zbtconeTracker.get();
    CMintMeta meta;
    if (!zbtconeTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbtconeTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
