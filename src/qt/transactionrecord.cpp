...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zbtconeTracker->HasMintTx(hash))) {
            //zBTCONE stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakezBTCONE;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zBTCONE;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IszBTCONEType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakezBTCONE:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zBTCONE:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakezBTCONE || type == TransactionRecord::MNReward) {
