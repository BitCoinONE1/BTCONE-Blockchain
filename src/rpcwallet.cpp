...
UniValue getzerocoinbalance(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zBTCONE balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zBTCONE balance.\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nList all zBTCONE mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"zBTCONE ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zbtconeTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (const CMintMeta& m : setMints) {
            ...
            objMint.push_back(Pair("zBTCONE ID", m.hashPubcoin.GetHex()));     // PubCoin
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zbtconeTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zBTCONE mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nMint the specified zBTCONE amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Piv to convert to zBTCONE\n"
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zBTCONE is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "\nSpend zBTCONE to a BTCONE address.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in BTCONE.\n"
            "      \"address\": \"xxx\"         (string) BTCONE address or \"zerocoinmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zBTCONE
    ...
    return DoZbtconeSpend(nAmount, fMintChange, fMinimizeChange, nSecurityLevel, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nSpend zBTCONE mints to a BTCONE address.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    return DoZbtconeSpend(nAmount, false, true, 100, vMintsSelected, address_str);
}


extern UniValue DoZbtconeSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, const int nSecurityLevel, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzBTCONETracker* zbtconeTracker = pwalletMain->zbtconeTracker.get();
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzBTCONETracker* zbtconeTracker = pwalletMain->zbtconeTracker.get();
    set<CMintMeta> setMints = zbtconeTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zBTCONE\n"
            ...
            "    \"v\": version,   (numeric) The version of the zBTCONE\n"
            "    \"k\": \"privkey\"  (string) The zBTCONE private key (V2+ zBTCONE only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzBTCONETracker* zbtconeTracker = pwalletMain->zbtconeTracker.get();
    set<CMintMeta> setMints = zbtconeTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() == 0)
        throw runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zBTCONE value of zerocoin mints that were added\n"
            "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zbtconeTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zBTCONE list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzbtconeseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw runtime_error(
            "setzbtconeseed \"seed\"\n"
            "\nSet the wallet's deterministic zbtcone seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zbtcone seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzbtconeseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzbtconeseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzBTCONEWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzbtconeseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "getzbtconeseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zBTCONE seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzbtconeseed", "") + HelpExampleRpc("getzbtconeseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zBTCONE seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zBTCONE to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zBTCONE to generate.\n"
    ...
    CzBTCONEWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzbtconestate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "dzbtconestate\n"
                        "\nThe current state of the mintpool of the deterministic zBTCONE wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzbtcone_count", nCount));
...
void static SearchThread(CzBTCONEWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedTozBTCONE(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzbtcone(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw runtime_error(
            "searchdzbtcone\n"
            "\nMake an extended search for deterministically generated zBTCONE that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zBTCONE to start with.\n"
            "2. \"range\"       (numeric) How many zBTCONE to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzbtcone", "1, 100, 2") + HelpExampleRpc("searchdzbtcone", "1, 100, 2"));
    ...
    boost::thread_group* dzbtconeThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzbtconeThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzbtconeThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zbtconeTracker->GetSerialHashes());
