...
public:
    ...
    bool WritezBTCONESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzBTCONESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzBTCONESeed_deprecated(uint256& seed);
    bool ErasezBTCONESeed();
    bool ErasezBTCONESeed_deprecated();

    bool WritezBTCONECount(const uint32_t& nCount);
    bool ReadzBTCONECount(uint32_t& nCount);
