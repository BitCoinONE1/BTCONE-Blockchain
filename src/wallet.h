...
// Possible states for zBTCONE send
enum ZerocoinSpendStatus {
    zBTCONE_SPEND_OKAY = 0,                            // No error
    zBTCONE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    zBTCONE_WALLET_LOCKED = 2,                         // Wallet was locked
    zBTCONE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    zBTCONE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    zBTCONE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    zBTCONE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    zBTCONE_TRX_CREATE = 7,                            // Everything related to create the transaction
    zBTCONE_TRX_CHANGE = 8,                            // Everything related to transaction change
    zBTCONE_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    zBTCONE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    zBTCONE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    zBTCONE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    zBTCONE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    zBTCONE_SPENT_USED_zBTCONE = 14,                      // Coin has already been spend
    zBTCONE_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    zBTCONE_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreatezBTCONEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzBTCONEWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBTCONETracker> zbtconeTracker;
    ...
    void setZWallet(CzBTCONEWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbtconeTracker = std::unique_ptr<CzBTCONETracker>(new CzBTCONETracker(strWalletFile));
    }

    CzBTCONEWallet* getZWallet() { return zwalletMain; }
    ...
    /** zBTCONE reset */
    boost::signals2::signal<void()> NotifyzBTCONEReset;
