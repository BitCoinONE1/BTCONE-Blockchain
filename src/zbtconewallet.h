...
#ifndef BTCONE_zBTCONEWALLET_H
#define BTCONE_zBTCONEWALLET_H
...
public:
    CzBTCONEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministiczBTCONE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedTozBTCONE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BTCONE_zBTCONEWALLET_H
