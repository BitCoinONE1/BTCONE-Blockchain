...
*Setup instructions for a gitian build of BTCONE using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BTCONE
...
- [Building BTCONE](#building-bitcoinone)
...
Clone the git repositories for bitcoinone and gitian and then checkout the bitcoinone version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoinone-crypto/bitcoinone.git
cd bitcoinone
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitcoinone repository
...
To build BTCONE (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcoinone repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinone/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitcoinone-crypto/bitcoinone
...
For example:
```bash
URL=https://github.com/crowning-/bitcoinone.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitcoinone=${COMMIT} --url bitcoinone=${URL} ../bitcoinone/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinone=${COMMIT} --url bitcoinone=${URL} ../bitcoinone/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinone=${COMMIT} --url bitcoinone=${URL} ../bitcoinone/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinone-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinone-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitcoinone-build.assert
```
...
[bitcoinone/gitian.sigs](https://github.com/bitcoinone-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
