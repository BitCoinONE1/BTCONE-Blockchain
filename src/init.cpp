...
CzBTCONEWallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BTCONE and zBTCONE money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 41472, 41474));
    ...
    strUsage += HelpMessageOpt("-btconestake=<n>", strprintf(_("Enable or disable staking functionality for BTCONE inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbtconestake=<n>", strprintf(_("Enable or disable staking functionality for zBTCONE inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzbtcone=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBTCONE minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbtconebackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBTCONE backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizebitcoinoneamount=<n>     " + strprintf(_("Keep N BTCONE anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 441473, 41475));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //BTCONE: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BTCONE specific: zerocoin and spork DB's
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculatezBTCONEMinted();
                        RecalculatezBTCONESpent();
                    }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzBTCONEWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zBTCONEWallet
        uiInterface.InitMessage(_("Syncing zBTCONE wallet..."));

        bool fEnableZPivBackups = GetBoolArg("-backupzbtcone", true);
        pwalletMain->setZPivAutoBackups(fEnableZPivBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbtconeTracker->Init();
...
//    }
//
//    nAnonymizeBitCoinOneAmount = GetArg("-anonymizebitcoinoneamount", 0);
